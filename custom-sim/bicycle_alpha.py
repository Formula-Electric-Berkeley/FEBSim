...
class Vehicle:
    ...
    def tire_model_simple(self, alpha, Fz):
        B = -8.526272038118725
...
class BicycleModel:
    ...
    def opt_mintime(self, curvatures, mesh_size, init_state=None, segment_number = None):
        ...
        for k in range(N):
            ...
            # DRIVEN GEAR IS SLOWER BY FACTOR OF R
            dt_opt.append(sf_opt[0] + sf_opt[1] + sf_opt[2])
            motor_speed = self.veh["gear_ratio"]*(Xk[7] + Xk[8]) / (2) * wheel_scale
            ...
            # TODO----WRONG--------WHY GEAR RATIO
            known_motor_speed = self.veh["gear_ratio"]*(Xk[7] + Xk[8]) / (2) * wheel_scale
        ...
        # solver options
        opts = {"expand": True, "ipopt.max_iter": 10000, "ipopt.tol": 1e-7}
...
class SweepWrapper:
    def __init__(self, tracks, param_sweep, parts, mesh=0.25):
        ...
        self.parts = parts
        self.mesh = 2.5        # make a mesh variable for each track
...
def main_test():
    ...
    param_sweep = {
                ...
                "re": [0.29, 0.32, 0.35, 0.38, 0.41],              # Effective wheel radius, meters
                "mu": [2.3]
    }
