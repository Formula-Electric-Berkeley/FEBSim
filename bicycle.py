...
def state_vars(nx, v_s, beta_s, omega_z_s, n_s, xi_s, wheel_scale):
    ...
    return v, omega_z, omega_z_s, wfl, wrl, n, x, xi, x_s, beta
...
def control_vars(nu, delta_s, torque_drive_s, f_brake_s, gamma_x_s):
    ...
    return gamma_x, 
    # gamma_y, 
    delta, f_brake, kappa, torque_drive, u_s, u


def normal_forces(gamma_x):
    # F_Nfr = (mass * g * lr) / (L * 2) + gamma_x - gamma_y
    F_Nfl = (mass * g * lr) / (L * 2) + gamma_x
...
def wheel_frame_vels(vx_fl, vy_fl, 
                    #  vx_rr, vy_rr, 
                     vx_rl, vy_rl, delta):
    ...
    return {
        # "front right long": vlfr, 
        # "front right lat": vtfr, 
        "front left long": vlfl, 
        "front left lat": vtfl, 
        # "rear right long": vlrr, 
        # "rear right lat": vtrr, 
        "rear left long": vlrl, 
        "rear left lat": vtrl
    } 
...
def slip_angles(vtfl, vlfl, 
    ...
    return {
        # "front right": alpha_fr, 
        "front left": alpha_fl, 
        # "rear right": alpha_rr, 
        "rear left": alpha_rl
    }

def slip_ratios(wfl, wrl, 
                # vlfr, 
                vlfl, 
                # vlrr, 
                vlrl):
...
def body_frame_forces(Flfl, Ftfl, Flrl, Ftrl, delta):
...
def brake_forces(f_brake):
    ...
    return {
        # "front right": F_brake_fr, 
        "front left": F_brake_fl, 
        # "rear right": F_brake_rr, 
        "rear left": F_brake_rl
    }


def brake_torques(F_brake_fl, F_brake_rl):
    ...
    return {
        # "front right": torque_brake_fr, 
        "front left": torque_brake_fl, 
        # "rear right": torque_brake_rr, 
        "rear left": torque_brake_rl
    }
...
def opt_mintime(cd):
    ...
    v, omega_z, omega_z_s, wfl, wrl, n, x, xi, x_s, beta = state_vars(nx, v_s, beta_s, omega_z_s, n_s, xi_s, wheel_scale)
    ...
    gamma_x, delta, f_brake, kappa, torque_drive, u_s, u = control_vars(nu, delta_s, torque_drive_s, f_brake_s, gamma_x_s)
    ...
    Fy = (
        ...
        Fy_fl + 
        # Fy_rr + 
        Fy_rl)  # net force in the y (lateral car direction)
    ...
    return delta_s, torque_drive_s, f_brake_s, v_s, beta_s, omega_z_s, n_s, wheel_scale, mesh_point_separations, kappa, kappa_interp, gamma_x_s, x, xi_s, x_s, dx, nx, u, u_s, sf, tau, nu, N, B, C, D, d, body_F
