...
def opt_mintime():
        ...
        # velocity [m/s]
        v_n = ca.SX.sym('v_n')          # we give CasADi's solver normalized variables to improve convergence
        ...
        alpha_fr = -ca.arctan(vtfr / np.abs(vlfr)) #TODO: old code used the smooth_abs helper function; replace if fucked
        alpha_fl = -ca.arctan(vtfl / np.abs(vlfl))
        alpha_rr = -ca.arctan(vtrr / np.abs(vlrr))
        alpha_rl = -ca.arctan(vtrl / np.abs(vlrl))

        # compute the slip ratios
        sigma_fr = (re * wfr - vlfr) / np.abs(vlfr)
        sigma_fl = (re * wfl - vlfl) / np.abs(vlfl)
        sigma_rr = (re * wrr - vlrr) / np.abs(vlrr)
        sigma_rl = (re * wrl - vlrl) / np.abs(vlrl)

        # compute wheel grip forces in the wheel frame
        Flfr, Ftfr = combined_slip_forces(sigma_fr, alpha_fr, F_Nfr) #Format: F_longitudinal_front_right
        ...
        # drag forces
        Fx_aero = - veh.drag_coeff * vx * np.abs(vx) #TODO add rolling resistance here later

        # compute the net force and torque on the vehicle
        Fx = Fx_fr + Fx_fl + Fx_rl + Fx_rr + Fx_aero                                            # net force in the x (longitudinal car direction)
        Fy = Fy_fr + Fy_fl + Fy_rr + Fy_rl                                                      # net force in the y (lateral car direction)
        Kz = lf*(Fy_fr + Fy_fl) - lr*(Fy_rr + Fy_rl) + wf/2*(Fx_fr - Fx_fl) + wr/2*(Fx_rr - Fx_rl)      #net torque about z axis
        ...
        wrr_dot = sf*(-Flrr*re + torque_brake_rr + torque_drive/2) / (Jw + Je*veh.ratio_final/2) #gear ratio
        wrl_dot = sf*(-Flrl*re + torque_brake_rl + torque_drive/2) / (Jw + Je*veh.ratio_final/2)
        ...
        xi_min = - 0.25 * np.pi / xi_s                  # min. relative angle to tangent of reference line [rad]
        xi_max = 0.25 * np.pi / xi_s                    # max. relative angle to tangent of reference line [rad]
        ...
        f_fx = ca.Function('f_fx', [x, u], [Fx_fl, Fx_fr, Fx_rl, Fx_rr],
                        ['x', 'u'], ['Fxfl', 'Fxfr', 'Fxrl', 'Fxrr'])
        # lateral tire forces in the vehicle frame [N]
        f_fy = ca.Function('f_fy', [x, u], [Fy_fl, Fy_fr, Fy_rl, Fy_rr],
        ...
        for k in range(N): 
                ...
                if k > 0: 
                        ...
                        sf_k = (1 - kappa_interp(k) * Xk[3] * n_s) / (Xk[0] * v_s * np.cos(Xk[1] * beta_s + Xk[4] * xi_s) ) 

                        # limit the rate of change *in time* between consecutive steering/drive/brake commands
                        g.append((Uk - w[1+(d+2)*(k-1)]) / (mesh_point_separations[k - 1] * sf_k))
        ...
        df0 = np.concatenate((t_opt, x_opt), axis=1)            # combine t_opt with our state solution
