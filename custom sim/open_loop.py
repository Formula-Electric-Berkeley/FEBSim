...
def simulate():
    ...
    overall_regen_efficiency = 0.5                      # assumes xx% of energy from braking is regenerated by the motor
    regen_power_limit = 20                               # maximum rate of regen in kW
    regen_power_limit = regen_power_limit * 3.6*10**6   # in Joules
    #TODO: model this better with more EECS data later

    '''
    Brake force: 
    1/2 mv^2 -> do KE analysis for rough values
    '''
    
    brake_force = np.where(brake_force < 0.0, 0.0, brake_force) #bps is always positive, unless we overshoot, in which case it's -1
    
    regen_power = np.where(regen_power > regen_power_limit, regen_power_limit, regen_power)
    
    regen_power = brake_force * overall_regen_efficiency 
    ...
    regenenerated_energy = np.trapz(regen_power, x = time)


    energy_cost_total = number_of_laps*total_work/(3.6*10**6)       # in kWh
    energy_gained_total = number_of_laps*regenenerated_energy/(3.6*10**6)   # in kWh
